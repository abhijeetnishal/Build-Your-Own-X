...
export default function Home() {
  ...
  const handleSubmit = async(e: React.MouseEvent<HTMLButtonElement>)=>{
    ...
    if(!userName || !password || !confirmPassword){
        ...
        setMessage('Weak password! Ensure it has 8 or more characters with 1 uppercase, 1 lowercase, 1 digit and 1 special character');
    }
    else if(password !== confirmPassword){
        setMessage('Password and confirm password fields should match')
    }
    else{
        const response = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/auth/register`,{
            method: 'POST',
            headers: {
  ...
  <main className="w-full h-[calc(100dvh-30px)] flex flex-row xl:flex-row lg:flex-row md:flex-row sm:flex-col xs:flex-col">
    ...
    <section className="w-1/2 xl:w-1/2 lg:w-1/2 md:w-1/2 sm:w-full xs:w-full h-full flex flex-col pl-[100px] xl:pl-[100px] lg:pl-[100px] md:pl-[100px] sm:pl-[0px] xs:pl-[0px] justify-center items-center bg-black text-white">
      ...
      <section className="w-full flex flex-col xs:items-center pt-[30px]">
        ...
        <section className="w-full h-[160px] flex flex-col justify-around xs:items-center">
          ...
          <section className='flex flex-row justify-center items-center w-[350px] xl:w-[350px] lg:w-[350px] md:w-[250px] sm:w-[350px] xs:w-[250px] h-[40px] outline-none bg-black border-[2px] border-gray-600 rounded-[4px]'>
              <input placeholder='Password' type={isPasswordEyeBtnClicked? 'text' : 'password'} value={password} onChange={(event)=>{setPassword(event.target.value)}} className='w-[320px] xl:w-[310px] lg:w-[310px] md:w-[210px] sm:w-[350px] xs:w-[210px] bg-black pl-2 outline-none' />
          ...
          <section className='flex flex-row justify-center items-center w-[350px] xl:w-[350px] lg:w-[350px] md:w-[250px] sm:w-[350px] xs:w-[250px] h-[40px] outline-none bg-black border-[2px] border-gray-600 rounded-[4px]'>
              <input placeholder='Confirm password' type={isConfirmPasswordEyeBtnClicked? 'text' : 'password'} value={confirmPassword} onChange={(event)=>{setConfirmPassword(event.target.value)}} className='w-[320px] xl:w-[310px] lg:w-[310px] md:w-[210px] sm:w-[310px] xs:w-[210px] bg-black pl-2 outline-none' />
              <button className='pr-[5px]' onClick={onConfirmPasswordEyeBtnClickFunc}>
                  {
      ...
      <section className="flex flex-col pt-[40px] pb-[10px]">
        ...
        <Link href='/login' className="w-[350px] xl:w-[350px] lg:w-[350px] md:w-[250px] sm:w-[350px] xs:w-[250px] h-[35px] flex justify-center items-center border-[2px] border-gray-600 rounded-[20px]">
